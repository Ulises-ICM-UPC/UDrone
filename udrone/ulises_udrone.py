#
# Thu Mar 10 15:10:15 2022, extract from Ulises by Gonzalo Simarro
#
import copy
import cv2
import datetime
...
def DisplayCRInImage(img, cs, rs, options={}): # 202109141700 # *** 
    ...
    return imgOut
def DistanceFromAPointToAPoint(x0, y0, x1, y1): # 202109280945
    ''' comments:
    .- input x0 and y0 are floats or float-ndarrays
    .- input x1 and y1 are floats or float-ndarrays
        .- if both ({x0, y0} and {x1, y1}) are ndarrays, they have the same length
...
def LengthOfASegment(segment): # 202110211410
    ''' comments:
    .- input segment is a dictionary (including at least 'x0', 'y0', 'x1' and 'y1')
    .- output length is a float
    '''
    length = DistanceFromAPointToAPoint(segment['x0'], segment['y0'], segment['x1'], segment['y1'])
    return length
def LengthOfASegmentOfAPolyline(polyline, posSegment): # 202110211409
    ''' comments:
    .- input polyline is a dictionary (including at least 'xs' and 'ys')
    .- input posSegment is an integer
    .- output lengthOfASegment is a float
    '''
    lengthOfASegment = LengthOfASegment(SegmentOfAPolyline(polyline, posSegment))
    return lengthOfASegment
def LengthsOfSegmentsOfAPolyline(polyline): # 202110211410
    ''' comments:
    .- input polyline is a dictionary (including at least 'xs' and 'ys'=
    .- output lengthsOfSegments is a float-list
    '''
    lengthsOfSegments = [LengthOfASegmentOfAPolyline(polyline, pos) for pos in range(NOfSegmentsOfAPolyline(polyline))]
...
def Matrix2Array(M, options={}): # 202109141800 # *** 
    ...
    return A
def MeanAndSigmaOfImages(pathsImages): # 202110211135
    ''' comments:
    .- pathsImages is a list of strings (paths of input images)
    '''
    for posPathImage, pathImage in enumerate(pathsImages):
        imgH, n, np1 = cv2.imread(pathImage).astype(float), 1. * posPathImage, 1. * posPathImage + 1.
        if posPathImage == 0:
            imgSigma = 0. * imgH
            imgMean = imgH
        else:
    ...
    imgSigma = (np.round(2. * imgSigma)).astype(np.uint8)
...
def NForRANSAC(eRANSAC, pRANSAC, sRANSAC): # *** 
    ...
    return N
def NOfSegmentsOfAPolyline(polyline): # 202110211407
    ''' comments:
    .- input polyline is a dictionary (including at least 'xs' and 'ys')
    .- output nOfSegments is an integer
    '''
    assert len(polyline['xs']) == len(polyline['ys']) > 1
    nOfSegments = len(polyline['xs']) - 1
    return nOfSegments
...
def ScaledSubsetVariables2MainSet(dataBasic, scaledSubsetVariables, subsetVariablesKeys, subCsetVariablesDictionary, nc, nr, options={}): # *** 
    ...
    return mainSet
def SegmentOfAPolyline(polyline, posSegment): # 202110211408
    ''' comments:
    .- input polyline is a dictionary (including at least 'xs' and 'ys')
    .- input posSegment is an integer so that 0 <= posSegment <= nOfSegments - 1
    .- output segment is a dictionary (including 'x0', 'y0', 'x1' and 'y1')
    '''
    assert 0 <= posSegment <= NOfSegmentsOfAPolyline(polyline) - 1
    segment = {}
    segment['x0'], segment['x1'] = polyline['xs'][posSegment], polyline['xs'][posSegment + 1]
    segment['y0'], segment['y1'] = polyline['ys'][posSegment], polyline['ys'][posSegment + 1]
    return segment
def SegmentsOfAPolyline(polyline): # 202110211409
    ''' comments:
    .- input polyline is a dictionary (including at least 'xs' and 'ys')
    .- output is a list of segments (see SegmentOfAPolyline)
    '''
    segments = [SegmentOfAPolyline(polyline, pos) for pos in range(NOfSegmentsOfAPolyline(polyline))]
    return segments
