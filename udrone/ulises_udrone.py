#
# Thu Sep 30 12:25:24 2021, extract from Ulises by Gonzalo Simarro and Daniel Calvete
#
import copy 
import cv2 
import datetime 
...
def ApplyHomographyHa01(Ha01, xs0, ys0): # 202109141200
    ''' comments:
    .- input Ha is a 3x3-float-ndarray
...
def ErrorH(chs, rhs, horizonLine): # 202109131100
    ...
    return errorH
def ErrorT(dataForCal, mainSet, options={}): # 202109131100
    ''' comments:
    .- input dataForCal is a dictionary (including at least 'xs', 'ys', 'zs', 'cs', 'rs' and 'aG')
    ...
    keys = ['chs', 'rhs', 'aH']
    if set(keys) <= dataForCal.keys() and all([dataForCal[item] is not None for item in keys]) and len(dataForCal['chs']) == len(dataForCal['rhs']) > 0 and dataForCal['aH'] > 1.e-8: # account for errorH
...
def FindAFirstSeed(dataBasic, dataForCal, subsetVariablesKeys, subCsetVariablesDictionary, options={}): # 202109241556
    ...
    '''
    keys, defaultValues = ['timedelta', 'xc', 'yc', 'zc'], [datetime.timedelta(seconds=100.), None, None, None]
    options = CompleteADictionary(options, keys, defaultValues)
    ...
    mainSetSeed, errorTSeed, time0, imgDiagonal = None, 1.e+11, datetime.datetime.now(), np.sqrt(nc ** 2 + nr ** 2)
    ...
    while datetime.datetime.now() - time0 < options['timedelta']: # IMP*
        scaledSubsetVariables = GenerateRandomScaledVariables(dataBasic, subsetVariablesKeys, options={key:dataForCal[key] for key in ['nc', 'nr', 'xc', 'yc', 'zc']})
        try: # IMP* to try
        ...
        if not IsVariablesOK(scaledSubsetVariables, subsetVariablesKeys):
            continue
        if errorT < 1.0 * imgDiagonal: # IMP* WATCH OUT
            try:
            ...
            if errorT < 0.2 * imgDiagonal: # IMP* WATCH OUT
                break
    return mainSetSeed, errorTSeed
def FindGoodPositionsForHomographyHa01ViaRANSAC(xs0, ys0, xs1, ys1, parametersRANSAC): # 202109131700
    ''' comments:
    ...
    if len(xs0) == len(ys0) == len(xs1) == len(ys1) >= 4:
        ...
        for iN in range(N):
            ...
            if Ha01 is None: # aligned points
                continue
        ...
        possGood = None
    return possGood
def FindHomographyHa01(xs0, ys0, xs1, ys1): # 202109131700
    ''' comments:
    .- input xs0, ys0, xs1 and ys1 are float-ndarrays of the same length
    ...
    if len(xs0) == len(ys0) == len(xs1) == len(ys1) >= 4:
        ...
        Ha = None
    return Ha
def FindHomographyHa01ViaRANSAC(xs0, ys0, xs1, ys1, parametersRANSAC): # 202109131700
    ''' comments:
    .- input xs0, ys0, xs1 and ys1 are float-ndarrays of the same length
    .- input parametersRANSAC is a dictionary (including at least 'p', 'e', 's' and 'errorC')
        .- p is the probability that all points are good (0.9999)
    ...
    Ha = FindHomographyHa01(xs0[possGood], ys0[possGood], xs1[possGood], ys1[possGood])
    return Ha, possGood
def GenerateRandomScaledVariables(dataBasic, variablesKeys, options={}): # 202109241335
    ...
    '''
    keys, defaultValues = ['nc', 'nr', 'xc', 'yc', 'zc'], None
    options = CompleteADictionary(options, keys, defaultValues)
    variablesDictionary = {}
    for key in variablesKeys:
        if key in ['xc', 'yc', 'zc']: # dataBasic['referenceValuesDictionary'][key] is None
            value0 = options[key]
        elif key in ['oc', 'or']: # dataBasic['referenceValuesDictionary'][key] is None
            value0 = N2K(options['n' + key[-1]])
        else:
...
def LoadDataBasic0(options={}): # 202109271434
    ...
    '''
    keys, defaultValues = ['selectedVariablesKeys'], [['xc', 'yc', 'zc', 'ph', 'sg', 'ta', 'k1a', 'sca']]
    options = CompleteADictionary(options, keys, defaultValues)
...
def LoadDataPdfTxt(options={}): # 202109101200
    ...
    '''
    def Length2NOfPixels(length, ppm): # 202109101200
        ''' comments:
        ...
        return nOfPixels
    def NOfPixels2Length(nOfPixels, ppm): # 202109101200
        ''' comments:
    ...
    elif options['pathFile'] is not None:
        pathFile = options['pathFile']
    else:
        pathPlanviews, station, date0OfTheWorld = [options['dataBasic'][item] for item in ['pathPlanviews', 'station', 'date0OfTheWorld']]
        pathFile = pathPlanviews + os.sep + station + date0OfTheWorld + 'pdf' + options['planview'] + '.txt'
    if pathFile is not None:
        rawData = ReadRectangleFromTxt(pathFile, {'c1':1, 'valueType':'float'})
        assert all([len(rawData) == 7, np.min(rawData[3:]) > 0.])
        data = {}
    ...
    data['nc'] = Length2NOfPixels(data['xYLengthInC'], data['ppm'])
    data['nr'] = Length2NOfPixels(data['xYLengthInR'], data['ppm'])
    assert np.min([data['nc'], data['nr']]) > 5
    data['xYLengthInC'] = NOfPixels2Length(data['nc'], data['ppm'])
    data['xYLengthInR'] = NOfPixels2Length(data['nr'], data['ppm'])
    ...
    data['polylineU'] = {'xs':xsU, 'ys':ysU}
    data['polylineD'] = {'xs':xsD, 'ys':ysD}
    data['polyline0'] = {'xs':xs0, 'ys':ys0}
    data['polyline1'] = {'xs':xs1, 'ys':ys1}
    if options['rewrite']:
...
def NonlinearManualCalibration(dataBasic, dataForCal, subsetVariablesKeys, subCsetVariablesDictionary, options={}): # 202109010000 *** 
    ...
    log10E = np.log10(max([errorTSeed, 1.]))
...
def NonlinearManualCalibrationForcingUniqueSubCset(dataBasic, ncs, nrs, css, rss, xss, yss, zss, chss, rhss, subsetVariabless, subsetVariablesKeys, subCsetVariabless, subCsetVariablesKeys, options={}): 
    ...
    .- input css, rss, xss, yss, zss, chss, rhss, subsetVariabless and subCsetVariabless are float-ndarrays-lists
    .- input subsetVariablesKeys is a string-list
...
def ObtainGoodGCPsRANSAC(dataBasic, dataForCal, subsetVariablesKeys, subCsetVariablesDictionary, options={}): # 202109281641
    ...
    .- input dataForCal is a dictionary (including at least 'cs', 'rs', 'xs', 'ys' and 'zs')
    .- input subsetVariablesKeys is a string-list
    .- input subCsetVariablesDictionary is a dictionary
    ...
    for iOfRANSAC in range(options['nOfRANSAC']):
        ...
        if 'mainSetSeeds' in dataForCalH.keys():
            ...
            dataForCalH['mainSetSeeds'] = mainSetSeeds
        ...
        mainSet, errorT = NonlinearManualCalibration(dataBasic, dataForCalH, subsetVariablesKeys, subCsetVariablesDictionary, options={'verbose':False}) #! set FALSE IN GENERAL
        if mainSet is not None:
            ...
            if len(possGoodH) > len(possGood):
                ...
                if options['verbose']:
                    print('')
                    print('... RANSAC iteration {:3}: {:3} good lines ({:})'.format(iOfRANSAC+1, len(possGood), possGood+1))
    print('')
    return possGood
def PerturbateScaledVariables(dataBasic, scaledVariables, variablesKeys, options={}): # 202109241340
    ...
    variablesDictionary = Array2Dictionary(variablesKeys, variables)
    for key in variablesKeys:
        variablesDictionary[key] = variablesDictionary[key] + options['perturbationFactor'] * Random(-1., +1.) * dataBasic['referenceRangesDictionary'][key]
    variables = Dictionary2Array(variablesKeys, variablesDictionary)
    scaledVariables = VariablesScaling(dataBasic, variables, variablesKeys, 'scale')
    return scaledVariables
...
def ReadAFirstSeed(dataBasic, dataForCal, subsetVariablesKeys, subCsetVariablesDictionary, mainSetSeeds): # 202109241556
    ...
    return mainSetSeed, errorTSeed
def ReadCalTxt(pathFile): # 202109271348
    ''' comments:
    ...
    .- output errorT is a float
    '''
    rawData = np.asarray(ReadRectangleFromTxt(pathFile, {'c1':1, 'valueType':'float'}))
...
def ReadCdgTxt(pathFile): # 202109271404
    ''' comments:
    .- input pathFile is a string
    .- output cs, rs, xs, ys and zs are float-ndarrays
        .- it does not load negative cs or rs
    '''
    rawData = np.asarray(ReadRectangleFromTxt(pathFile, {'c1':5, 'valueType':'float'}))
    if len(rawData) == 0:
    ...
    return cs, rs, xs, ys, zs
def ReadCdhTxt(pathFile): # 202109271415
    ''' comments:
    .- input pathFile is a string
    .- output chs and rhs are float-ndarrays
        .- it does not load negative chs or rhs    
    '''
    rawData = np.asarray(ReadRectangleFromTxt(pathFile, {'c1':2, 'valueType':'float'}))
    if len(rawData) == 0:
...
def UpdateSubCsetVariables(dataBasic, ncs, nrs, css, rss, xss, yss, zss, chss, rhss, subsetVariabless, subsetVariablesKeys, subCsetVariablesSeed, subCsetVariablesKeys, options={}): # 202109271810
    ...
    print('... error in the optimization of the unique parameters: {:9.3f}'.format(errorTO), end='-> ', flush=True)
...
def VariablesScaling(dataBasic, variables, variablesKeys, direction): # 202109241439
    ...
    if direction == 'scale':
        ...
        assert False
    return variables
def Video2Snaps(pathVideo, pathFolderSnaps, fps, options={}): # 202109271245
    ''' 
    .- input pathVideo is a string
    .- input pathFolderSnaps is a string
    .- input fps is a float (desired frames per second)
    '''
    keys, defaultValues = ['extension'], ['png']
    options = CompleteADictionary(options, keys, defaultValues)
    MakeFolder(pathFolderSnaps)
    fnVideo = pathVideo[pathVideo.rfind(os.sep)+1:pathVideo.rfind('.')]
    video = cv2.VideoCapture(pathVideo)
    fpsOfVideo = video.get(cv2.CAP_PROP_FPS)
    fps = min([fpsOfVideo / int(fpsOfVideo / fps), fpsOfVideo])
    milisecond, optionsSnap = 0., {key:options[key] for key in ['extension']}
    hasFrame = WriteASnap(video, fnVideo, milisecond, pathFolderSnaps, options=optionsSnap)
    while hasFrame:
        milisecond = milisecond + 1000. / fps
        hasFrame = WriteASnap(video, fnVideo, milisecond, pathFolderSnaps, options=optionsSnap)
    return None
def WriteASnap(video, fnVideo, milisecond, pathFolderSnaps, options={}): # 202109271244
    ''' comments:
    ...
    pathSnap = pathFolderSnaps + os.sep + '{:}_{:}.{:}'.format(fnVideo, str(int(milisecond)).zfill(12), options['extension'])
    hasFrame, img = video.read()
    if hasFrame:
        cv2.imwrite(pathSnap, img)
    return hasFrame
def WriteCalTxt(pathFile, allVariables, nc, nr, errorT): # 202109271348
    ''' comments:
    .- input pathFile is a string
    .- input allVariables is a 14-float-ndarray
    .- input nc and nr are integers or floats
    ...
    '''
    allVariablesKeys = ['xc', 'yc', 'zc', 'ph', 'sg', 'ta', 'k1a', 'k2a', 'p1a', 'p2a', 'sca', 'sra', 'oc', 'or'] # WATCH OUT order matters
    MakeFolder(pathFile[0:pathFile.rfind(os.sep)])
